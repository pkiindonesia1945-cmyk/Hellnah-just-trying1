...
require('../kayzen.js')(conn, m, message, store);
...
async function Serialize(conn, m, store) {
    ...
    try {
        ...
        // daftar admin grup
        m.admins = participants.filter(p => p?.admin !== null).map(p => p.id || p.jid);

        // deteksi apakah pengirim admin
        m.isAdmin = Boolean(
            participants.find(
                p => p?.admin !== null && ((p.id && p.id === m.sender) || (p.jid && p.jid === m.sender))
            )
        );

        // simpan participant id
        m.participant = m.key.participant || "";

        // deteksi apakah bot admin
        m.isBotAdmin = Boolean(
            participants.find(
                p => p?.admin !== null && ((p.id && p.id === botNumber) || (p.jid && p.jid === botNumber))
            )
        );
